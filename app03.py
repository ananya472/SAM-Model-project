...
def upload_images():
    ...
    # Coordinates for segmentation
    if option == 'wall':
        point_coords = np.array([[100, 100]])  # Adjust as needed
        ...
        floor_point_coords = np.array([[450, 650]])  # Adjust as needed
        floor_point_labels = np.array([1])
        masks, _, _ = predictor.predict(point_coords=floor_point_coords, point_labels=floor_point_labels, multimask_output=False)

        if masks is not None and len(masks) > 0:
            floor_mask = masks[0]
            replacement_resized = cv2.resize(replacement_img, (floor_mask.shape[1], floor_mask.shape[0]))
            room_img[floor_mask > 0] = replacement_resized[floor_mask > 0]
